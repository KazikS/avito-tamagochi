...
//
// Соединение закрывается в любом случае, даже если под этим userID оно не
// числилось. Так задумано: Remove означает «это соединение закончилось», а
// userID — лишь подсказка, где его искать. Обратное поведение (закрывать
// только найденное) при ошибке вызывающего оставило бы открытый сокет,
// которого уже никто не держит.
...
//
// Соединения обходятся последовательно, поэтому в худшем случае вызов длится
// writeTimeout × (число вкладок пользователя). Это осознанно: вкладок обычно
// одна-две, а горутина на каждую запись стоила бы дороже, чем экономит. Звать
// Send из обработчика HTTP-запроса всё же не стоит — он для фонового пуша.
