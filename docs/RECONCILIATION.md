...
## ⚠️ Обновление: `feat/pet-service` больше не мержится этим планом

`internal/pet` и `internal/social` целиком написаны заново, в целевой раскладке
(`docs/ARCHITECTURE.md`), поверх `chore/contract-codegen`, а не переездом кода из
`feat/pet-service` — PR «питомец, WS-пуш, лидерборд, сводка дня». Причина: пока эта
ветка ждала реконсиляции, вертикальный срез оказался быстрее написать заново с
тестами против настоящего Postgres и защитой слоёв, чем чинить три известных дефекта
чеклиста (пустое тело `GetPetState`, роут без хендлера, гонка в `internal/websocket`)
и одновременно сводить его раскладку к целевой.

Из-за этого пункты ниже про `feat/pet-service` (`usecase/pet.go`, роуты питомца,
`internal/websocket` → `pkg/wsh`, `internal/models/pet.go`) — **не открытая работа**,
а решённые задним числом: `internal/pet` уже существует, уже в целевой раскладке, уже
чинит все три названных дефекта `pkg/wsh` (см. его докстринг). Саму ветку `feat/pet-service`
мержить не нужно — она конфликтовала бы с уже написанным, а не дополняла его.

Пункты про `feat/auth` (bcrypt, `NOT NULL` на пароле, расположение `User`, коллизия
миграций `users`) остаются реальной незакрытой работой: этот PR настоящую авторизацию
не строил, только demo-заглушку личности (`backend/cmd/demo_identity.go`, только при
`APP_ENV=demo`) — `login`+`password` из `feat/auth` всё ещё нужен, когда до него дойдут руки.
