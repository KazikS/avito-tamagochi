...
⚠️ Ставь **ту же минорную версию, что пинит CI** (её показывает `make doctor`).
Свежее — не лучше: у `gosec` и `staticcheck` между минорными версиями появляются
новые правила, и тогда `pre-push` краснеет на том, что в CI зелёное. `make doctor`
теперь предупреждает о таком расхождении.
