...
| **golangci-lint** | ту же, что пинит CI | `make lint`, `make verify`, гейт в CI | ставит `make setup` |
...
⚠️ **Не ставь линтер «посвежее» руками.** Официальная страница установки даёт
последнюю версию, а у четверых это будут четыре разных набора правил: `gosec` и
`staticcheck` добавляют проверки между минорными выпусками, и `pre-push` начинает
краснеть на том, что в CI зелёное. Именно так и случилось: `make setup` теперь
ставит ровно ту версию, которую пинит CI (`go install ...@vX.Y.Z`), а `make doctor`
предупредит, если в `PATH` оказалась другая.
