...
# Корень ФС и домашний каталог целиком. Проверяется регуляркой, а не глобом:
# шаблон *"rm -rf /"* — подстрока, он ловил ЛЮБОЙ абсолютный путь
# (`rm -rf /tmp/scratch` блокировался наравне с `rm -rf /`).
if printf '%s' "$cmd" | grep -qE 'rm[[:space:]]+-[a-zA-Z]*[rR][a-zA-Z]*[[:space:]]+(/|~)(\*)?([[:space:]]|;|&|\||$)'; then
